...
module.exports = class DevicePluginSwitcherPlugin extends Plugin {
  async onload() {
    ...
    if (this.settings.autoApplyOnStartup) {
      ...
      const timeout = window.setTimeout(() => {
        this.applyDeviceState({ manual: false });
      }, delay);
  ...
  async loadSettings() {
    ...
    this.settings.assignments = Object.assign({}, DEFAULT_SETTINGS.assignments, loaded.assignments || {});
    // Hidden defaults: keep the plugin simple and safe for synced .obsidian setups.
    this.settings.applyMode = "session";
    this.settings.autoApplyOnStartup = true;
    this.settings.applyDelayMs = 1500;
    this.settings.protectedPluginIds = [selfId];
  }
  ...
  getInstalledPlugins() {
    ...
    const selfId = this.manifest?.id || "device-plugin-switcher";
    const rows = values
      .filter((manifest) => manifest && manifest.id && manifest.id !== selfId)
      .map((manifest) => ({
        id: manifest.id,
  ...
  isPluginEnabled(pluginId) {
    ...
    if (manager?.enabledPlugins?.has && manager.enabledPlugins.has(pluginId)) {
      return true;
    }

    if (manager?.plugins && Object.prototype.hasOwnProperty.call(manager.plugins, pluginId)) {
      return true;
    }
  ...
  getAssignment(plugin) {
    ...
    if ([MODE_BOTH, MODE_DESKTOP, MODE_MOBILE, MODE_DISABLED].includes(existing)) {
      return existing;
    }
  ...
  getDesiredEnabledSet() {
    ...
    for (const plugin of plugins) {
      ...
      if (plugin.isDesktopOnly && isMobile) {
        continue;
      }

      if (mode === MODE_BOTH) {
        desired.add(id);
      } else if (mode === MODE_DESKTOP && isDesktop) {
        desired.add(id);
      } else if (mode === MODE_MOBILE && isMobile) {
        desired.add(id);
      }
    }
  ...
  async setAssignment(pluginId, mode) {
    if (![MODE_BOTH, MODE_DESKTOP, MODE_MOBILE, MODE_DISABLED].includes(mode)) return;
  ...
  async resetAssignmentsFromCurrentState() {
    ...
    for (const plugin of plugins) {
      if (plugin.id === (this.manifest?.id || "device-plugin-switcher")) {
        this.settings.assignments[plugin.id] = MODE_BOTH;
      } else if (plugin.isDesktopOnly) {
        this.settings.assignments[plugin.id] = plugin.enabled ? MODE_DESKTOP : MODE_DISABLED;
      } else {
        this.settings.assignments[plugin.id] = plugin.enabled ? MODE_BOTH : MODE_DISABLED;
      }
    }
...
class DevicePluginSwitcherSettingTab extends PluginSettingTab {
  ...
  renderTable() {
    ...
    const plugins = query
      ? allPlugins.filter((plugin) => {
          return plugin.name.toLowerCase().includes(query) || plugin.id.toLowerCase().includes(query);
        })
      : allPlugins;
  ...
  getCounts(plugins) {
    ...
    for (const plugin of plugins) {
      const mode = this.plugin.getAssignment(plugin);
      if (mode === MODE_BOTH) counts.both += 1;
      else if (mode === MODE_DESKTOP) counts.desktop += 1;
      else if (mode === MODE_MOBILE) counts.mobile += 1;
